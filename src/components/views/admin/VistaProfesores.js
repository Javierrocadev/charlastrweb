...
const VistaProfesores=()=>{
    ...
    useEffect(()=>{
        const fechData = async ()=>{
            try{
                ...
                console.log(responseProfesores);
      ...
      <div class="p-1.5 min-w-full inline-block align-middle">
        <div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden dark:bg-accent-200 dark:border-gray-700">
          ...
          <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            ...
            {profesores.filter((usuario) => usuario.idRole === 2).map((usuario, index)=>(
              <tr key={index} class="bg-white hover:bg-gray-50 dark:bg-accent-200 dark:hover:bg-slate-800">
                  ...
                  <div class="block p-6" >
                    <div class="flex items-center gap-x-3">
                      ...
                      <span class="block text-sm font-semibold text-gray-800 dark:text-gray-200">{usuario.nombre}</span>
                        <span class="block text-sm text-accent-100">{usuario.apellidos}</span>
                      </div>
                    </div>
                  </div>
                </td>
                ...
                <td class="h-px w-72 max-w-[2rem] align-top">
                  <div class="block p-6" >
                    ...
                    <span class="block text-sm text-accent-100">{usuario.telefono}</span>
                  </div>
                </td>
                <td class="h-px w-px whitespace-nowrap align-top">
  ...
  <span className="py-1 px-1.5 inline-flex items-center gap-x-1 text-xs font-medium bg-red-100 text-teal-800 rounded-full dark:bg-teal-500/10 dark:text-teal-500">
    baja
            ...
            class="py-2 px-3 w-full inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-red-600 bg-red-600 text-white shadow-sm hover:bg-red-800 duration-300 disabled:opacity-50 disabled:pointer-events-none "
          >
            Desactivar
          </button>
) : (
    ...
    type="button"
    onClick={() =>axiosApi.usuarios.updateEstadoUsuario(usuario.idUsuario,0)}
     class="py-2 px-3 w-full inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-green-600 bg-green-600 text-white shadow-sm hover:bg-green-800 duration-300 disabled:opacity-50 disabled:pointer-events-none "
  >
    Activar
  </button>
)}
